...
% Tone number, as used in Wade-Giles.  Set as a real superscript in the
% running font rather than relying on the precomposed characters, which not
% every font has.
...
\section{Language families of the region}

\begin{frame}{Official Languages}
  \centering
  \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{pics/image1.png}
\end{frame}
...
\begin{frame}{Austronesian}
  ...
  found on numerous islands in the eastern and central Pacific Ocean
...
\item Filipino (Tagalog) (50 million, 30 million as a first language).
\end{itemize}
...
\begin{frame}
  ...
  \item This reflects the cultural and historical unity of China and the use of a common script
...
\begin{frame}{Mandarin Romanization Comparison}
  ...
  \begin{tabular}{lllll}
Characters & Wade-Giles & Hanyu Pinyin & Bopomofo & Notes\\ \hline
中国/中國 & Chung\tn{1}-kuo\tn{2} & Zhōngguó & ㄓㄨㄥ ㄍㄨㄛˊ & China\\
北京 & Pei\tn{3}-ching\tn{1} & Běijīng & ㄅㄟˇ ㄐㄧㄥ & PRC Capital \\
台北 & T'ai\tn{2}-pei\tn{3} & Táiběi & ㄊㄞˊ ㄅㄟˇ & RoC Capital \\
毛泽东/毛澤東 & Mao\tn{2} Tse\tn{2}-tung\tn{1} & Máo Zédōng & ㄇㄠˊ ㄗㄜˊ ㄉㄨㄥ & Mao Tse-tung \\
蒋介石/蔣介石 & Chiang\tn{3} Chieh\tn{4}-shih\tn{2} & Jiǎng Jièshí & ㄐㄧㄤˇ ㄐㄧㄝˋ ㄕˊ & Chiang Kai-shek \\
孔子 & K'ung\tn{3} Tzu\tn{3} & Kǒng Zǐ & ㄎㄨㄥˇ ㄗˇ & Confucius \\
  \end{tabular}\endgroup
  \bigskip


  \begin{itemize}
  \item \txx{Wade–Giles} — An older system for romanizing Mandarin Chinese, once widely used in English-language scholarship, still used for names in Taiwan.
  \item \txx{Hanyu Pinyin} — The official romanization system for Standard Mandarin in China and the international standard, using Latin letters and diacritics or numbers to mark tones.
  \item \txx{Bopomofo (Zhuyin)} — A phonetic writing system for
    Mandarin using a dedicated set of symbols (ㄅㄆㄇㄈ etc.),
    especially widely used in Taiwan for teaching pronunciation.
  \end{itemize}
...
\begin{itemize}
\item Use  `Tai' (pronounced with an unaspirated dental stop \emph{t}) for the family name
\item Use `Thai' (pronounced with an aspirated dental stop \emph{th}) for the national language of Thailand.
\end{itemize}
...
Japanese and Korean share very similar syntax (agglutinative, case marking, verb final, pre-modification) and both have borrowed much vocabulary from Chinese.
 ...
 \begin{center}
    \begin{tabular}{lll}
      ...
      Korean     & \textkorean{학교} (學校)   & \textit{hakgyo} \\ \hline
%      Vietnamese & học hiệu (學校)    & \textit{học hiệu} \\
    \end{tabular}
  \end{center}

\end{frame}

\section{Mainland Southeast Asia as a linguistic area}
...
\begin{frame}{Classifiers in Thai and Mandarin}
    \begin{itemize}
        \item \textbf{Mandarin Chinese} classifiers:
            \begin{itemize}
                \item \zhs{一只猫} (yī zhī māo) – ``one (zhī) cat''
                \item \zhs{三本书} (sān běn shū) – ``three (běn) books''
                \item \zhs{五辆车} (wǔ liàng chē) – ``five (liàng) cars''
            \end{itemize}
        \item \textbf{Thai} classifiers:
            \begin{itemize}
                \item \ths{เด็กหนึ่งคน} (dèk nùeng khon) – ``one (khon) child''
                \item \ths{บ้านสองหลัง} (bâan sǎawng lǎng) – ``two (lǎng) houses''
                \item \ths{เสื้อผ้าสามตัว} (sûea phâa sǎam tua) – ``three (tua) shirts''
            \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Shared terms for ``rice'' at different stages}:
            \begin{itemize}
                \item \textbf{Thai}:
                    \begin{itemize}
                        \item \tha{khao} (\ths{ข้าว}) – rice in general
                        \item \tha{khao niao} (\ths{ข้าวเหนียว}) – sticky rice
                    \end{itemize}
        ...
        \item \textbf{Kinship terms emphasizing relative age}:
            \begin{itemize}
                ...
                \item \textbf{Thai}:
                    \begin{itemize}
                        \item \ths{พี่} (phîi) – older sibling
                        \item \ths{น้อง} (nóng) – younger sibling
                    \end{itemize}
            \end{itemize}
...
\begin{frame}{Sentence-Final Particles}
    \begin{itemize}
        ...
        \item Example: Thai particle \ths{นะ} (na) to soften statements
...
\section{Geography, history and language policy}
...
\section{Comparing the features}
