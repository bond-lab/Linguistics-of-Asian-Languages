...
\usepackage{fontspec}
\usepackage{xeCJK}
% CJK has no italic, so map it to upright rather than let XeTeX fake-slant it
% (\mtcitestyle italicises, and \jpn{}/\zh{} put Han characters inside it).
% The sans family needs declaring too: beamer's body font is sans.
\setCJKmainfont{Noto Sans CJK SC}[ItalicFont=Noto Sans CJK SC]
\setCJKsansfont{Noto Sans CJK SC}[ItalicFont=Noto Sans CJK SC]
\setCJKmonofont{Noto Sans Mono CJK SC}[ItalicFont=Noto Sans Mono CJK SC]
% The bopomofo tone marks sit in Spacing Modifier Letters, not the Bopomofo
% block, so xeCJK leaves them to Latin Modern -- which has no ˊ or ˋ.
\xeCJKDeclareCharClass{CJK}{"02C7, "02C9, "02CA, "02CB, "02D9}
% Noto Sans CJK SC carries no Hangul at all, so Korean needs the KR face.
\newCJKfontfamily\korfont{Noto Sans CJK KR}[ItalicFont=Noto Sans CJK KR]
\newcommand{\hg}[1]{{\korfont #1}}
\newfontfamily\Libertine[Mapping=tex-text]{Linux Libertine O}
% Latin Modern Sans has no small caps, so \textsc{sg} in the glosses was
% silently coming out as lowercase "sg". Biolinum is Libertine's sans
% companion and does have them.
...
% Tone number, as used in Wade-Giles. Latin Modern Sans has no ⁴, so set them
% all as real superscripts rather than relying on precomposed characters.
...
extending into SEA.
\item \textbf{Korean, Japanese and Ainu}: Korean Peninsula and Japan
\end{itemize}  
MSEA forms a linguistic area in which many languages share similar
features despite being genetically unrelated.
...
\end{minipage} &
 ...
 \end{minipage} 
  ...
  Much of Mainland Southeast Asia is very mountainous.  The people of the
  uplands have long resisted being governed by central states of the lowland
  basins --- what \citet{scott:2009} calls \txx{Zomia}.
%   James ScoZ's\\
...
\begin{frame}
  \begin{center}
    \includegraphics[width=0.9\textwidth,height=0.9\textheight,keepaspectratio]{pics/image5.png}
  \end{center}
\end{frame}
...
\begin{frame}{Austronesian}
  ...
  Austronesian is the most widespread family in the world, with
  somewhere around 1,200 languages altogether and 300 million native
  speakers. Aside from Southeast Asia, Austronesian languages are
  found on numerous islands in the eastern and central Paciﬁc Ocean
  all the way to Easter Island. There is also a western outpost
  language (Malagasy), spoken on the island of Madagascar.
...
\item Filipino (Tagalog) (50 million, 30 million as a ﬁrst language).
\end{itemize}
...
\item Northern Mon-Khmer, which divides into Palaungic and Khmuic, spoken in
...
spoken in northern Vietnam, and in Guangxi and Yunnan, China.
...
\item Mandarin
  ...
  \item Southwest (Hubei, Sichuan, Yunnan, Guizhou, western Hunan, western Guangxi)
  \end{itemize}
...
\item Gan (Jiangxi, eastern Hunan, SE Hubei)
...
\begin{frame}
  According to \citet[p.~38]{goddard:2005}
  \begin{itemize}
...
(Li 1992, cited in \citealt{goddard:2005}). 
...
中国/中國 & Chung\tn{1}-kuo\tn{2} & Zhōngguó & China\\ 
北京 & Pei\tn{3}-ching\tn{1} & Běijīng & PRC Capital \\
    台北 & T'ai\tn{2}-pei\tn{3} & Táiběi & RoC Capital \\
                                 
毛泽东/毛澤東 & Mao\tn{2} Tse\tn{2}-tung\tn{1} & Máo Zédōng & Mao \\
蒋介石/蔣介石 & Chiang\tn{3} Chieh\tn{4}-shih\tn{2} & Jiǎng Jièshí & Chiang Kai-shek \\
孔子 & K'ung\tn{3} Tzu\tn{3} & Kǒng Zǐ & Confucius \\ 
...
% 孔子 & K'ung³ Tzu³ & Kǒng Zǐ & ㄎㄨㄥˇ ㄗˇ \\ \hline
% \end{tabular}
\end{frame}
...
The Tai family splits into three branches:
  \begin{itemize}
\item Northern
...
have been spoken by a single group of people approximately fifteen
...
\begin{frame}
  \begin{center}
    \includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{pics/image8.png}
  \end{center}
\end{frame}

\begin{frame}{Japanese, Korean, and Ainu Families}
  \begin{itemize}
  \item Japonic: Japanese (125 million speakers) 
  \item Koreanic: Korean (70 million) 
  \item Ainu (isolate, effectively dormant; a handful of elderly speakers, with active revitalisation) 
  \end{itemize}
...
\begin{itemize}
\item \txx{Sinoxenic vocabulary}: whole strata of Chinese loans, each
  with regular, predictable local readings.
  \\ Ainu has no such layer --- its loans come from Japanese.
\end{itemize}
  
\end{frame}

\begin{frame}{Sinoxenic vocabulary}

  Chinese words borrowed wholesale into Japanese, Korean and Vietnamese,
  each language reading the characters in its own regular way.

  \begin{center}
    \begin{tabular}{lll}
      \textbf{Language} & \textbf{Script} & \textbf{Transliteration} \\ \hline
      Mandarin   & 学校 (學校)        & \textit{xuéxiào} \\
      Japanese   & 学校               & \textit{gakkō} \\
      Korean     & \hg{학교} (學校)   & \textit{hakgyo} \\
      Vietnamese & học hiệu (學校)    & \textit{học hiệu} \\ \hline
    \end{tabular}
  \end{center}

  All from 學 \textit{learn} + 校 \textit{school}. Vietnamese dropped the
  characters in the 20th century, so its script \emph{is} the
  transliteration.

\end{frame}

\begin{frame}{Sinoxenic vocabulary: the correspondences are regular}

  \begin{itemize}
  \item Vietnamese has the reading, but everyday `school' is
    \vie{trường học} \mbox{(場學)} --- the stratum is there, the usage
    differs.
  \item The same morphemes recur right across the modern vocabulary:
    \begin{itemize}
    \item 大學 `university': \textit{dàxué} / \textit{daigaku} /
      \textit{daehak} / \textit{đại học}
    \item 電話 `telephone': \textit{diànhuà} / \textit{denwa} /
      \textit{jeonhwa} / \textit{điện thoại}
    \end{itemize}
  \item 學 is consistently \textit{gaku} / \textit{hak} / \textit{học},
    大 consistently \textit{dai} / \textit{dae} / \textit{đại}: borrowing
    a writing system means borrowing a whole lexicon with it.
  \item And the traffic is not one way --- 電話 was coined in Japan and
        ...
        \item \textbf{Lexical tone examples}:
            \begin{itemize}
                ...
                \item \textbf{Thai} has five tones:
                    \begin{itemize}
                        \item \textit{khaa} (be stuck) – mid tone
                        \item \textit{khàa} (galangal) – low tone
                        \item \textit{khâa} (servant, I) – falling tone
                        \item \textit{kháa} (trade) – high tone
                        \item \textit{khǎa} (leg) – rising tone
                    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Shared terms for "rice" at different stages}:
            \begin{itemize}
                \item \textbf{Thai}:
                    \begin{itemize}
                        \item \thaig{khao} (\tha{ข้าว}) – rice in general
                        \item \thaig{khao niao} (\tha{ข้าวเหนียว}) – sticky rice
                    \end{itemize}
                  \item \textbf{Vietnamese}:
                    \begin{itemize}
                    \item \vie{lúa} – rice plant
                    \item \vie{gạo} – uncooked rice grains
                    \item \vie{cơm} – cooked rice
                    \end{itemize}
                  \item \textbf{Japanese}:
                    \begin{itemize}
                    ...
                    \end{itemize}
                  \item \textbf{Indonesian}:
                    \begin{itemize}
                    \item \ind{padi} – rice plant (still growing in the field)
                    \item \ind{gabah} – harvested but unhusked rice
                    \item \ind{beras} – husked, uncooked rice grains
                    \item \ind{nasi} – cooked rice
                    \end{itemize}
                  \end{itemize}
                  \framebreak
...
\begin{frame}[allowframebreaks]{Official languages and multilingualism}

  \begin{description}
  \item[Singapore] Four official languages: English, Malay, Mandarin and
    Tamil. Malay is the \emph{national} language and the anthem is sung in
    it, but English is the language of administration and the medium of
    education; every child also studies an assigned ``mother tongue''.
  \item[Indonesia] \textit{Bahasa Indonesia} is the sole national language,
    standardised from Riau Malay and adopted by the Youth Pledge of 1928.
    It is the first language of only a minority, and that is rather the
    point --- it belonged to no dominant group. Javanese has far more
    native speakers (about 80 million) and no official status. Some 700
    local languages.
  \item[Philippines] Filipino (based on Tagalog) and English are official;
    the roughly 180 other languages are ``auxiliary official languages in
    the regions''. Mother-tongue-based multilingual education was brought
    into the early grades in 2012 and remains contested.
  \item[China] The 2000 Law on the Standard Spoken and Written Chinese
    Language makes Putonghua and simplified characters the standard for
    government, education and broadcasting. Minority languages have
    constitutional protection; Sinitic ``dialects'' such as Cantonese and
    Hokkien have none.
  \item[Timor-Leste] Tetum and Portuguese are official, Indonesian and
    English are ``working languages''. Around 30 local languages, mostly
    Austronesian with a handful of Papuan ones.
  \end{description}

  \bigskip

  Official status, national identity, and what people actually speak at
  home are three different things.
...
\begin{frame}{The spread of tone in MSEA languages}
  ...
  demonstrated by the French linguist André Haudricourt
  \citep{haudricourt:1954}. Some
...
\begin{frame}{Summary I}
  ...
  \begin{itemize}
  \item There are five major linguistic families in Mainland SEA.
  \item There are many languages spoken in the region
  \item However, linguistic diversity is  relatively low
  \item Probably due to centuries of language contact and human
    population movement.
  \end{itemize}
\end{frame}
