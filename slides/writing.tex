...
\begin{frame}{Cia-Cia}
  ...
  \item In 2007  Professor Chun Tai-hyun, a professor of Malay and Indonesian linguistics at Hankuk University of Foreign Studies in Seoul developed a Hangeul orthography
    \begin{itemize}
    \item For fricative /v/ it uses the obsolete jamo \textkorean{ㅸ} \textit{kapyeounpieup} `light pieup' 
    \end{itemize}
...
\end{frame}
\begin{frame}{Cia-cia being used}
 \begin{center}
   \includegraphics[width=0.7\textwidth]{pics/ciacia.png}
 \end{center}
 From \href{https://languagemuseum.org/language-of-the-month-november-2024-cia-cia/}{Language of the Month November 2024 – Cia-Cia} By L S, 
The National Museum of Language (online virtual museum)
\end{frame}
...
\begin{tabular}{llllll}
  ...
  Cantonese & Jyutping &  English & Mandarin & Pinyin\\
  \hline
...
\item Hiragana: Used for particles, auxiliary verbs, and suffixes.
  \begin{itemize}
  \item Derived from cursive versions of kanji with suitable pronunciation
  \end{itemize}
    \item Katakana: Used for loanwords, mimetics, and emphasis.
  \begin{itemize}
  \item Derived from components  of kanji with suitable pronunciation
  \end{itemize}
\end{itemize}
\end{frame}
...
\end{frame}
\begin{frame}{Katakana - a true syllabic alphabet} 
% Script developed in 10th century from Chinese characters. 52 characters.
...
\begin{frame}{Extending Hiragana and Katakana}
  ...
  \begin{itemize}
   ...
   \item \txx{Handakuten} (\textjapanese{半濁点: ゜}) plosive
    ...
    \end{itemize}
 \item \txx{Yōon} (\textjapanese{拗音}): palatalized
   \begin{itemize}
...
\begin{itemize}
    ...
    \item Example: \textjapanese{明} can be read as \textjapanese{あか} in  the native Japanese word  \textjapanese{明るい} \jpn[bright]{akarui}.
...
\begin{itemize}
    ...
    \end{itemize}
    \item Japanese orthography is still a challenging system to master.
\end{itemize}
\end{frame}

\section{Vietnamese}
\begin{frame}{Writing in Vietnam}

\begin{itemize}
\item Vietnamese today uses the Latin script-based Vietnamese alphabet
  to represent native Vietnamese words (thuần Việt)
\item From 111 BC up to the 20th century, Vietnamese literature was
  written in Văn ngôn (Classical Chinese) using chữ Hán (Chinese
  characters)
\item From the 13th century to 20th century it could also be written
  in Nôm (Chinese and original Vietnamese characters adapted for
  vernacular Vietnamese)
\item From the 17th century Portuguese missionaries developed a script
  based on Portuguese, which was mainly used within the Catholic
  community. During the French administration, the alphabet was
  further modified and then later made a part of compulsory education
  in 1910
\end{itemize}
\end{frame}

\begin{frame}{Thuần Việt}

\begin{itemize}
\item 29 letters:  7 letters use diacritics: ⟨ă⟩, ⟨â⟩, ⟨ê⟩, ⟨ô⟩, ⟨ơ⟩, ⟨ư⟩, and ⟨đ⟩
\item An additional 5 diacritics used to designate tone (as in ⟨à⟩,
  ⟨á⟩, ⟨ả⟩, ⟨ã⟩, and ⟨ạ⟩), level tone is unmarked.
\item Diacritics stack, e.g. \vi{nhất} ``first''
\item Sound changes in the spoken language have led to different letters, digraphs and trigraphs now representing the same sounds.
  \begin{itemize}
  \item Northern dialect has 6 tones 
  \item Southern dialect has merged ⟨ả⟩ \vi{hỏi} and ⟨ã⟩ \vi{ngã} to just ⟨ả⟩ \vi{hỏi}.
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Summary}
  ...
  \begin{itemize}
  \item There are many, many different scripts, with adaptations and
    borrowings
  \item It is very hard to change a nation's script although it does happen
    \begin{itemize}
    \item Most countries have abandoned or limited the use of Chinese Characters
    \item But having to learn a new script and reprint everything is a massive undertaking, \ldots
    \end{itemize}
  \end{itemize}
